...
# ============================================================
# Test coverage of calibration approximation ranges
#
# This is deliberately distinct from B-spline support.
# ============================================================

def analyze_calibration_coverage(
    test_inputs,
    ranges,
):
    coverage = []
...
#
# Only the spline term is evaluated here. The residual branch
# is preserved separately.
# ============================================================

@torch.no_grad()
def evaluate_spline_edges(
    layer,
    input_feature,
    xs,
):
...
#
# Polynomial is expressed in local coordinate
#
#     t = x - left
#
# so
...
#
# Important WDBC semantics:
#
# The approximation domain for each feature is
#
#   calibration range INTERSECT spline support.
#
# Outside true spline support, the spline contribution is
# exactly zero.
#
# If a test activation is outside the calibration approximation
# range but still inside spline support, the nearest end
...
def convert_layer_approx(
    ...
    for i in range(din):
        ...
        if xmax <= xmin:
            raise RuntimeError(
                f"layer feature {i}: "
                f"calibration range has no "
                f"intersection with spline support: "
                f"calib=["
                f"{calib_mins[i].item()}, "
                f"{calib_maxs[i].item()}], "
                f"support=["
                f"{support_min[i].item()}, "
                f"{support_max[i].item()}]"
            )
...
#
# The full extended knot vector defines 11 intervals for the
# cubic grid-size-5 KANLib spline.
# ============================================================
...
#
# Both approximate and exact representations obey the true
# finite support of the B-spline:
#
#     spline(x) = 0
#
# outside [support_min, support_max].
#
# For approximate representations, values inside true support
# but outside the calibration approximation interval use the
# nearest end polynomial.
# ============================================================

def polynomial_layer_forward(
    x,
    converted,
):
    ...
    for i in range(din):
        ...
        # Piece selection.
        #
        # Values below/above the approximation domain are
        # clamped to the first/last piece. Thus, if they remain
        # inside true spline support, that end polynomial is
        # extrapolated.
        #
        ...
        #
        # True spline support semantics.
        #
        # KANLib's spline contribution is zero outside the
        # extended knot-vector support.
        #
        valid = (
            (xi >= support_min[i])
            & (xi < support_max[i])
    ...
    #
    # Preserve residual SiLU branch exactly.
    #
    if weight_residual is not None:
...
def evaluate_representation(
    ...
    for i, layer in enumerate(model):
        #
        # Original layer receives original activation.
        #
        y_orig = layer(x_orig)
        ...
        #
        # Both original and transformed implementations receive
        # exactly the same original layer input.
        #
        ...
        #
        # The transformed layer receives transformed activations
        # from the preceding transformed layer.
        #
    ...
    transformed_pred = (
        transformed_logits.argmax(dim=1)
    )
    ...
    #
    # Binary classification logit margin.
    #
    original_margin = (
        original_logits[:, 1]
        - original_logits[:, 0]
...
# ============================================================
# Test coverage of calibration ranges
# ============================================================
...
coverage = analyze_calibration_coverage(
    test_inputs,
    ranges,
)
...
# ============================================================
# Print approximation-domain information
# ============================================================
...
for layer_index, layer in enumerate(model):
    ...
    print(
        f"  layer {layer_index}: "
        f"global approx range "
        f"[{approx_min.min().item():.6f}, "
        f"{approx_max.max().item():.6f}]"
    )
...
print(
    "END-TO-END WDBC CLASSIFICATION COMPARISON"
)
