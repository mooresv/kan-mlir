...
lowers `kan.piecewise_poly_linear` to a single GPU kernel. One CUDA thread
computes one `(batch, output)` element. Within the thread, the generated
code selects the knot interval, evaluates each cubic polynomial with
Horner's method, accumulates across input features, and writes the output.
...
compiler-produced fatbinary and launched through the CUDA Driver API with
persistent device-resident input, output, boundary, and coefficient
...
The generated GPU implementation has been validated against the independent
piecewise-polynomial reference over all benchmarked batch sizes. The maximum
absolute and RMS errors were zero in these tests.

## Preliminary H100 Performance

The initial performance experiment uses a KANLib layer with 2 inputs,
8 outputs, grid size 5, and cubic B-spline edge functions. All timings
below use an NVIDIA H100 80 GB HBM3 GPU after 100 warmup iterations and
1,000 measured iterations.

| Batch | KANLib layer (us) | Generated kernel (us) | Compiled layer (us) | Layer speedup |
|------:|------------------:|----------------------:|--------------------:|--------------:|
| 256 | 208.833 | 2.807 | 2.816 | 74.15x |
| 1,024 | 213.007 | 2.869 | 2.903 | 73.36x |
| 4,096 | 218.245 | 3.038 | 3.105 | 70.30x |
| 16,384 | 208.705 | 3.398 | 3.381 | 61.73x |
| 65,536 | 209.745 | 6.601 | 6.541 | 32.06x |

The compiled layer adds essentially no GPU-side overhead beyond the
generated kernel itself. Host submission cost for a compiled invocation is
approximately 1.6 us and is nearly independent of batch size.

Nsight Systems profiling of one warmed-up KANLib forward shows that the
reference implementation decomposes this small layer into 32--33 GPU
kernels, whereas the compiler-generated implementation uses one kernel:

| Batch | KANLib kernels | KANLib aggregate kernel time (us) | Compiled kernels | Generated kernel time (us) | Kernel-time reduction |
|------:|---------------:|----------------------------------:|-----------------:|---------------------------:|----------------------:|
| 256 | 33 | 53.056 | 1 | 2.807 | 18.90x |
| 65,536 | 32 | 124.096 | 1 | 6.601 | 18.80x |

This separates two benefits of the compiler approach:

1. **Representation/lowering benefit.** Direct evaluation of the
   knot-aligned cubic representation reduces aggregate GPU execution time
   by about 19x at both batch-size extremes.
2. **Fusion/dispatch benefit.** The compiler replaces 32--33 fine-grained
   GPU kernels with one fused kernel, eliminating most of the launch and
   dependency overhead visible in the KANLib layer invocation.

The near-constant approximately 19x reduction in GPU kernel execution time
across a 256x increase in batch size is an important preliminary result:
the computational advantage of the transformed representation does not
disappear as the batch grows.
...
4. Validate the CPU lowering against synthetic and KANLib-derived references.
...
9. Validate generated GPU outputs against the independent polynomial reference.
...
The benchmark sweep uses one static-shape compiled kernel per batch size:
